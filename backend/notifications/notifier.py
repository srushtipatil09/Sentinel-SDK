...
class NotificationEngine:
    ...
    async def send_incident_alert(
        ...
        message_body = (
            f"🚨 *[{severity}] Observe AI Incident Alert*\n"
        ...
        try:
            if channel == "slack" and config.target_url:
                ...
                if has_resend or has_smtp:
                    success = self._send_email(target_email, f"[{severity}] Observe AI Incident: {service_name}", message_body)
                    if not success:
        ...
        try:
            if channel == "slack" and config.target_url:
                ...
                success = await self._send_generic_webhook(config.target_url, {
                    "event": "TEST_ALERT",
                    "channel": channel,
                    "message": "Observe AI webhook channel verification successful."
                })
            elif channel == "email":
                target_email = (config.settings_json or {}).get("email") or config.target_url or settings.EMAILS_FROM_EMAIL
                recipient = target_email
                ...
                has_smtp = bool(settings.SMTP_HOST and settings.SMTP_USER)
                if has_resend or has_smtp:
                    success = self._send_email(target_email, "Observe AI - Channel Test Notification", test_message)
                    if not success:
    ...
    def send_password_reset_email(self, recipient_email: str, reset_token: str) -> bool:
        ...
        subject = "Observe AI - Password Reset Request"
        body = (
            f"Hello,\n\n"
            f"You requested a password reset for your Observe AI account.\n"
